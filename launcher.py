...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Light - Discord Bot")
    parser.add_argument("--start", "-s",
                        help="Starts Light",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Light in case of issues",
                        action="store_true")
    parser.add_argument("--update-red",
                        help="Updates Light (git)",
                        action="store_true")
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Light:")
        print("1. Update Light + requirements (recommended)")
        print("2. Update Light")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Light (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            print("Are you sure? This will wipe ALL your Light's installation "
...
def run_red(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Light. "
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Light...")
                continue
            else:
                if not autorestart:
                    break

    print("Light has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Light without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Light Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Light has been "
        ...
        print("1. Run Light /w autorestart in case of issues")
        print("2. Run Light")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Light needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Light cannot work without the pip module. Please make sure to "
